...
conf = {
    'last_refresh_url': 'https://himawari8-dl.nict.go.jp/himawari8/img/D531106/latest.json',  # latest photo
    # scale, time, row, col
...
def download(args):
    ...
    for row in range(scale):
        for col in range(scale):
            ...
            png.paste(tile, (550 * row, 550 * col,
                             550 * (row + 1), 550 * (col + 1)))
    if 'fout' in args:
        fpath = args['fout']
    else:
        fpath = "%s.png" % utf2local(args['time']).strftime(
            "%Y/%m/%d/%H%M%S").replace('/', '')
...
def get_last_image(fout=None, scale=1):
    ...
    print 'export[%s] scale[%i]' % (fout, scale)
...
def run(fout=None, scale=1):
    get_last_image(fout, scale)
...
if __name__ == '__main__':
    ...
    #     run(fout=sys.argv[1])
    # elif len(sys.argv) == 3:
    #     run(fout=sys.argv[1], scale=int(sys.argv[2]))
    run(fout='earth.png', scale=2)
